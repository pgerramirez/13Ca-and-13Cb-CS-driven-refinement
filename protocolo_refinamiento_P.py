...
def minimize(pdb_file, counter):
	...
	pose.pdb_info().name('refine_minimize')
	pymover.apply(pose)
	pymover.send_energy(pose)

	pose.dump_pdb("refine_minimize.pdb")
	pdb_file = "refine_minimize.pdb"
	return pdb_file
...
def nowalkers_fast(pdb_file, counter):
	...
	movemap.set_chi(True)
	walkers = GiveWalkers(pdb_file)
	fd.write("**WALKERS NWF %s : %s\n" % (counter, walkers))
	walkerlist.append(len(walkers))
	fe.write("%*s%*s scorefxn:%*s flaws:%*s carmsd:%*s aarmsd:%*s gdt:%*s\n" % (15, "nowalkers_fast", 4, counter, 14, str(scorefxn(pose)), 3, str(len(walkers)), 14, str(carmsd), 14, str(aarmsd), 14, str(gdt)))
	...
	pose.dump_pdb("refine_nowalkers%s.pdb" % counter)
	pdb_file = "refine_nowalkers%s.pdb" % counter
	return pdb_file
...
for i in range(0, iterations):
	...
	if i >= 4:
		...
		if flag == 0:
			...
			if walkerlist[-1] == walkerlist[-2] == walkerlist[-3] == walkerlist[-4] == walkerlist[-5] == walkerlist[-6] == walkerlist[-7] == walkerlist[-8] == walkerlist[-9] == walkerlist[-10] == walkerlist[-11]:
				...
				if nowalker_counter % 10 == 0 and nowalker_counter != 0:
					...
					fe.write("**Ten walker-free movements completed, protocol restarting.**\n")
					...
					fe.write("**Number of flaws unchanged in last 10 movements, allowing walker-free movement.**\n")
					pdb_file = nowalkers_fast(pdb_file, counter)
					nowalker_counter += 1
					flag = 1
	...
	if flag == 1:
		#fe.write("Ronda %s flag == 1\n" % i)
		if flaged_counter > 10:
			flag = 0
			flaged_counter = 0
		else:
			flaged_counter += 1
